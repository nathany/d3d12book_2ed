// Port of `Demos/C11_Stenciling` (StencilingApp.cpp/.h). The room mirror is marked in
// stencil, a reflected skull is restricted to those pixels with reflected lighting,
// the mirror is blended over it, and a projected planar shadow uses stencil to prevent
// double blending. Use A/D/W/S to move the skull.
//
// Build & run FROM THE REPO ROOT (shader path + DXC DLLs are resolved relative to it):
//   odin run odin_port/C11_Stenciling -debug
package c11_stenciling
...
Stenciling_App :: struct {
	...
	reflected_pass_cb:         common.Per_Pass_CB,

	skull_ritem:               ^Render_Item,
	reflected_skull_ritem:     ^Render_Item,
	shadowed_skull_ritem:      ^Render_Item,
	skull_translation:         [3]f32,
...
main :: proc() {
	...
	app.theta = 1.4 * math.PI
	app.phi = 0.4 * math.PI
	app.radius = 12.0
	app.draw_wireframe = false
	app.skull_translation = {0.0, 1.0, -5.0}
	...
	room_geo := build_room_geometry(&app, &upload_batch)
	app.geometries[room_geo.name] = room_geo
...
update :: proc(base: ^common.D3D_App) {
	...
	update_reflected_pass_cb(app)
}

// C++: TexWavesApp::OnKeyboardInput — empty in this demo.
on_keyboard_input :: proc(app: ^Stenciling_App) {
	dt := common.game_timer_delta_time(&app.timer)
	if u16(win.GetAsyncKeyState(win.INT('A'))) & 0x8000 != 0 {app.skull_translation.x -= dt}
	if u16(win.GetAsyncKeyState(win.INT('D'))) & 0x8000 != 0 {app.skull_translation.x += dt}
	if u16(win.GetAsyncKeyState(win.INT('W'))) & 0x8000 != 0 {app.skull_translation.y += dt}
	if u16(win.GetAsyncKeyState(win.INT('S'))) & 0x8000 != 0 {app.skull_translation.y -= dt}
	app.skull_translation.y = max(app.skull_translation.y, 0.0)

	skull_world := d3d_math.rotation_y(0.5 * math.PI) *
		d3d_math.scaling(0.45, 0.45, 0.45) *
		d3d_math.translation(
			app.skull_translation.x,
			app.skull_translation.y,
			app.skull_translation.z,
		)
	app.skull_ritem.world = skull_world

	reflection := d3d_math.matrix_reflect({0, 0, 1, 0})
	app.reflected_skull_ritem.world = skull_world * reflection

	to_main_light := d3d_math.Vec4{
		-app.main_pass_cb.lights[0].direction.x,
		-app.main_pass_cb.lights[0].direction.y,
		-app.main_pass_cb.lights[0].direction.z,
		0,
	}
	shadow := d3d_math.matrix_shadow({0, 1, 0, 0}, to_main_light)
	app.shadowed_skull_ritem.world = skull_world * shadow * d3d_math.translation(0, 0.001, 0)
}
...
animate_materials :: proc(app: ^Stenciling_App) {
}

// C++: TexWavesApp::UpdateCamera.
update_camera :: proc(app: ^Stenciling_App) {
...
update_per_object_cb :: proc(app: ^Stenciling_App) {
...
update_material_buffer :: proc(app: ^Stenciling_App) {
...
update_main_pass_cb :: proc(app: ^Stenciling_App) {
...
update_reflected_pass_cb :: proc(app: ^Stenciling_App) {
	app.reflected_pass_cb = app.main_pass_cb
	reflection := d3d_math.matrix_reflect({0, 0, 1, 0})
	for i in 0 ..< 3 {
		direction := app.main_pass_cb.lights[i].direction
		app.reflected_pass_cb.lights[i].direction = d3d_math.transform_normal(direction, reflection)
	}
	common.copy_data(&app.curr_frame_resource.pass_cb, 1, app.reflected_pass_cb)
}

// C++: TexWavesApp::UpdateImgui — Options panel with the wave sliders.
update_imgui :: proc(app: ^Stenciling_App) {
...
draw :: proc(base: ^common.D3D_App) {
	...
	// Mark visible mirror pixels with stencil value 1 without writing color.
	base.command_list->OMSetStencilRef(1)
	base.command_list->SetPipelineState(app.psos["markStencilMirrors"])
	draw_render_items(app, app.ritem_layer[.Mirrors][:])

	// Draw the reflected skull only where the mirror marked stencil.
	base.command_list->SetGraphicsRootConstantBufferView(
		u32(Gfx_Root_Arg.PASS_CBV),
		pass_cb->GetGPUVirtualAddress() + u64(app.curr_frame_resource.pass_cb.element_byte_size),
	)
	base.command_list->SetPipelineState(app.psos["drawStencilReflections"])
	draw_render_items(app, app.ritem_layer[.Reflected][:])

	// Restore main pass constants and stencil reference.
	base.command_list->SetGraphicsRootConstantBufferView(
		u32(Gfx_Root_Arg.PASS_CBV),
		pass_cb->GetGPUVirtualAddress(),
	)
	base.command_list->OMSetStencilRef(0)

	base.command_list->SetPipelineState(app.psos["transparent"])
	draw_render_items(app, app.ritem_layer[.Transparent][:])

	base.command_list->SetPipelineState(app.psos["shadow"])
	draw_render_items(app, app.ritem_layer[.Shadow][:])
...
draw_render_items :: proc(app: ^Stenciling_App, ritems: []^Render_Item) {
...
build_cbv_srv_uav_descriptor_heap :: proc(app: ^Stenciling_App) {
...
build_root_signature :: proc(app: ^Stenciling_App) {
...
// C++: StencilingApp::BuildShadersAndInputLayout — BasicTex.hlsl.
build_shaders_and_input_layout :: proc(app: ^Stenciling_App) {
	...
	app.shaders["standardVS"] = common.compile_shader("Shaders/BasicTex.hlsl", vs_args[:])
	app.shaders["opaquePS"] = common.compile_shader("Shaders/BasicTex.hlsl", ps_args[:])
...
build_psos :: proc(app: ^Stenciling_App) {
	...
	// Mark the mirror in stencil without writing color or depth.
	mark_mirrors_pso_desc := base_pso_desc
	mark_mirrors_pso_desc.BlendState.RenderTarget[0].RenderTargetWriteMask = 0
	mark_mirrors_pso_desc.DepthStencilState.DepthWriteMask = .ZERO
	mark_mirrors_pso_desc.DepthStencilState.StencilEnable = true
	mark_mirrors_pso_desc.DepthStencilState.FrontFace = {
		StencilFailOp = .KEEP, StencilDepthFailOp = .KEEP,
		StencilPassOp = .REPLACE, StencilFunc = .ALWAYS,
	}
	mark_mirrors_pso_desc.DepthStencilState.BackFace = mark_mirrors_pso_desc.DepthStencilState.FrontFace

	mark_mirrors: ^d3d12.IPipelineState
	common.hr_panic(
		app.device->CreateGraphicsPipelineState(
			&mark_mirrors_pso_desc,
			d3d12.IPipelineState_UUID,
			common.ptr(&mark_mirrors),
		),
		"CreateGraphicsPipelineState(markStencilMirrors)",
	)
	app.psos["markStencilMirrors"] = mark_mirrors

	// Reflections render only where stencil equals the mirror reference value.
	draw_reflections_pso_desc := base_pso_desc
	draw_reflections_pso_desc.DepthStencilState.StencilEnable = true
	draw_reflections_pso_desc.DepthStencilState.FrontFace = {
		StencilFailOp = .KEEP, StencilDepthFailOp = .KEEP,
		StencilPassOp = .KEEP, StencilFunc = .EQUAL,
	}
	draw_reflections_pso_desc.DepthStencilState.BackFace = draw_reflections_pso_desc.DepthStencilState.FrontFace
	draw_reflections_pso_desc.RasterizerState.CullMode = .BACK
	draw_reflections_pso_desc.RasterizerState.FrontCounterClockwise = true

	draw_reflections: ^d3d12.IPipelineState
	common.hr_panic(
		app.device->CreateGraphicsPipelineState(
			&draw_reflections_pso_desc,
			d3d12.IPipelineState_UUID,
			common.ptr(&draw_reflections),
		),
		"CreateGraphicsPipelineState(drawStencilReflections)",
	)
	app.psos["drawStencilReflections"] = draw_reflections

	// Transparent planar shadow; stencil increment prevents double blending.
	shadow_pso_desc := transparent_pso_desc
	shadow_pso_desc.DepthStencilState.StencilEnable = true
	shadow_pso_desc.DepthStencilState.FrontFace = {
		StencilFailOp = .KEEP, StencilDepthFailOp = .KEEP,
		StencilPassOp = .INCR, StencilFunc = .EQUAL,
	}
	shadow_pso_desc.DepthStencilState.BackFace = shadow_pso_desc.DepthStencilState.FrontFace

	shadow: ^d3d12.IPipelineState
	common.hr_panic(
		app.device->CreateGraphicsPipelineState(
			&shadow_pso_desc,
			d3d12.IPipelineState_UUID,
	...
	app.psos["shadow"] = shadow
}

// C++: TexWavesApp::BuildFrameResources.
build_frame_resources :: proc(app: ^Stenciling_App) {
	pass_count :: 2
...
build_materials :: proc(app: ^Stenciling_App) {
	common.material_lib_init(&app.mat_lib, &app.tex_lib)
	common.material_lib_add(
		&app.mat_lib, &app.tex_lib, "checkertile",
		"checkboardMap", "defaultNormalMap", "defaultGlossHeightAoMap",
		{1, 1, 1, 1}, {0.07, 0.07, 0.07}, 0.3,
	)
	common.material_lib_add(
		&app.mat_lib, &app.tex_lib, "icemirror",
		"iceMap", "defaultNormalMap", "defaultGlossHeightAoMap",
		{1, 1, 1, 0.6}, {0.1, 0.1, 0.1}, 0.5,
	)
	common.material_lib_add(
		&app.mat_lib, &app.tex_lib, "shadowMat",
		"defaultDiffuseMap", "defaultNormalMap", "defaultGlossHeightAoMap",
		{0, 0, 0, 0.5}, {0.01, 0.01, 0.01}, 0.0,
	)
}
...
build_render_items :: proc(app: ^Stenciling_App) {
	room_geo := app.geometries["roomGeo"]
	skull_geo := app.geometries["skullGeo"]
	identity := d3d_math.MAT4_IDENTITY

	add_render_item(
		app,
		.Opaque, identity, identity,
		common.material_lib_get(&app.mat_lib, "checkertile"),
		room_geo, room_geo.draw_args["floor"],
	)
	add_render_item(
		app,
		.Opaque, identity, identity,
		common.material_lib_get(&app.mat_lib, "bricks0"),
		room_geo, room_geo.draw_args["wall"],
	)
	app.skull_ritem = add_render_item(
		app,
		.Opaque, identity, identity,
		common.material_lib_get(&app.mat_lib, "skullMat"),
		skull_geo, skull_geo.draw_args["skull"],
	)
	app.reflected_skull_ritem = add_render_item(
		app,
		.Reflected, identity, identity,
		common.material_lib_get(&app.mat_lib, "skullMat"),
		skull_geo, skull_geo.draw_args["skull"],
	)
	app.shadowed_skull_ritem = add_render_item(
		app,
		.Shadow, identity, identity,
		common.material_lib_get(&app.mat_lib, "shadowMat"),
		skull_geo, skull_geo.draw_args["skull"],
	)
	add_render_item(
		app,
		.Mirrors, identity, identity,
		common.material_lib_get(&app.mat_lib, "icemirror"),
		room_geo, room_geo.draw_args["mirror"],
	)
	add_render_item(
		app,
		.Transparent, identity, identity,
		common.material_lib_get(&app.mat_lib, "icemirror"),
		room_geo, room_geo.draw_args["mirror"],
	)
}
...
build_room_geometry :: proc(
	app: ^Stenciling_App,
	upload_batch: ^common.Resource_Upload_Batch,
) -> ^common.Mesh_Geometry {
	vertices := [20]common.Model_Vertex{
		// Floor.
		{pos={-3.5, 0, -10}, normal={0, 1, 0}, tex_c={0, 4}},
		{pos={-3.5, 0,   0}, normal={0, 1, 0}, tex_c={0, 0}},
		{pos={ 7.5, 0,   0}, normal={0, 1, 0}, tex_c={4, 0}},
		{pos={ 7.5, 0, -10}, normal={0, 1, 0}, tex_c={4, 4}},
		// Left wall.
		{pos={-3.5, 0, 0}, normal={0, 0, -1}, tex_c={0, 2}},
		{pos={-3.5, 4, 0}, normal={0, 0, -1}, tex_c={0, 0}},
		{pos={-2.5, 4, 0}, normal={0, 0, -1}, tex_c={0.5, 0}},
		{pos={-2.5, 0, 0}, normal={0, 0, -1}, tex_c={0.5, 2}},
		// Right wall.
		{pos={2.5, 0, 0}, normal={0, 0, -1}, tex_c={0, 2}},
		{pos={2.5, 4, 0}, normal={0, 0, -1}, tex_c={0, 0}},
		{pos={7.5, 4, 0}, normal={0, 0, -1}, tex_c={2, 0}},
		{pos={7.5, 0, 0}, normal={0, 0, -1}, tex_c={2, 2}},
		// Upper wall.
		{pos={-3.5, 4, 0}, normal={0, 0, -1}, tex_c={0, 1}},
		{pos={-3.5, 6, 0}, normal={0, 0, -1}, tex_c={0, 0}},
		{pos={ 7.5, 6, 0}, normal={0, 0, -1}, tex_c={6, 0}},
		{pos={ 7.5, 4, 0}, normal={0, 0, -1}, tex_c={6, 1}},
		// Mirror.
		{pos={-2.5, 0, 0}, normal={0, 0, -1}, tex_c={0, 1}},
		{pos={-2.5, 4, 0}, normal={0, 0, -1}, tex_c={0, 0}},
		{pos={ 2.5, 4, 0}, normal={0, 0, -1}, tex_c={1, 0}},
		{pos={ 2.5, 0, 0}, normal={0, 0, -1}, tex_c={1, 1}},
	}
	indices := [30]u16{
		0, 1, 2, 0, 2, 3,
		4, 5, 6, 4, 6, 7,
	...
	geo.index_buffer_cpu = slice.clone(slice.to_bytes(indices[:]))
	common.create_static_buffer(
		upload_batch, raw_data(vertices[:]), len(vertices), size_of(common.Model_Vertex),
	...
	geo.vertex_byte_stride = size_of(common.Model_Vertex)
	geo.vertex_buffer_byte_size = u32(size_of(vertices))
	geo.index_format = .R16_UINT
	geo.index_buffer_byte_size = u32(size_of(indices))
	geo.draw_args["floor"] = {index_count=6, start_index_location=0, base_vertex_location=0, vertex_count=4}
	geo.draw_args["wall"] = {index_count=18, start_index_location=6, base_vertex_location=0, vertex_count=12}
	geo.draw_args["mirror"] = {index_count=6, start_index_location=24, base_vertex_location=0, vertex_count=4}
	return geo
}
