// Port of `Demos/C13_VecAddCS` (VecAddCS.cpp/.h). The sample keeps the rendered crate
// while adding a structured-buffer compute dispatch and CPU readback to results.txt.
//
//  - **Textures**: TextureLib loads DDS files through common/dds_loader.odin (the port's
//    replacement for DirectXTK12's CreateDDSTextureFromFile), each texture gets a
//    **bindless index** in the CbvSrvUav heap, and the material buffer carries those
//    indices so the pixel shader can do `ResourceDescriptorHeap[matData.DiffuseMapIndex]`
//    (SM 6.6 dynamic resources — no per-texture root bindings, ever).
//  - **The sampler heap**: seven fixed samplers at the SAM_* indices, created once in
//    d3d_app_init; shaders index SamplerDescriptorHeap[] directly. Both "directly
//    indexed" root-signature flags are required or the debug layer rejects the shader.
//  - **MaterialLib**: the material table moves into common (mirroring the C++), and the
//    per-frame material upload now includes MatTransform and the texture indices.
//
// Build & run FROM THE REPO ROOT (shader path + DXC DLLs are resolved relative to it):
//   odin run odin_port/C13_VecAddCS -debug
...
#assert(size_of(Data) == 20)

NUM_DATA_ELEMENTS :: 32
...
Vec_Add_CS_App :: struct {
	...
	input_buffer_a:            ^d3d12.IResource,
	input_buffer_b:            ^d3d12.IResource,
	output_buffer:             ^d3d12.IResource,
	read_back_buffer:          ^d3d12.IResource,
	buffer_index_a:            u32,
	buffer_index_b:            u32,
...
main :: proc() {
	...
	build_compute_buffers(&app, &upload_batch)
	...
	if app.input_buffer_a != nil {app.input_buffer_a->Release()}
	if app.input_buffer_b != nil {app.input_buffer_b->Release()}
	if app.output_buffer != nil {app.output_buffer->Release()}
	if app.read_back_buffer != nil {app.read_back_buffer->Release()}
...
on_keyboard_input :: proc(app: ^Vec_Add_CS_App) {
}

// C++: CrateApp::AnimateMaterials — empty in this demo.
animate_materials :: proc(app: ^Vec_Add_CS_App) {
}

// C++: CrateApp::UpdateCamera.
update_camera :: proc(app: ^Vec_Add_CS_App) {
...
update_per_object_cb :: proc(app: ^Vec_Add_CS_App) {
...
update_material_buffer :: proc(app: ^Vec_Add_CS_App) {
...
update_main_pass_cb :: proc(app: ^Vec_Add_CS_App) {
...
update_imgui :: proc(app: ^Vec_Add_CS_App) {
...
draw_render_items :: proc(app: ^Vec_Add_CS_App, ritems: []^Render_Item) {
...
build_cbv_srv_uav_descriptor_heap :: proc(app: ^Vec_Add_CS_App) {
...
build_root_signature :: proc(app: ^Vec_Add_CS_App) {
	...
	compute_serialized: ^d3d12.IBlob
	compute_error: ^d3d12.IBlob
	hr = d3d12.SerializeRootSignature(&compute_desc, ._1_0, &compute_serialized, &compute_error)
	if compute_error != nil {
		common.report_error(string(cstring(compute_error->GetBufferPointer())))
		compute_error->Release()
	}
	common.hr_panic(hr, "D3D12SerializeRootSignature(compute)")
	defer compute_serialized->Release()
	common.hr_panic(
		app.device->CreateRootSignature(
			0, compute_serialized->GetBufferPointer(), compute_serialized->GetBufferSize(),
...
build_shaders_and_input_layout :: proc(app: ^Vec_Add_CS_App) {
	...
	when ODIN_DEBUG {
		...
		cs_args := [?]string{"-E", "CS", "-T", "cs_6_6", dxc.ARG_DEBUG, dxc.ARG_SKIP_OPTIMIZATIONS}
	} else {
		vs_args := [?]string{"-E", "VS", "-T", "vs_6_6"}
		ps_args := [?]string{"-E", "PS", "-T", "ps_6_6"}
		cs_args := [?]string{"-E", "CS", "-T", "cs_6_6"}
	}
	...
	app.shaders["vecAddCS"] = common.compile_shader("Shaders/VecAdd.hlsl", cs_args[:])
...
build_psos :: proc(app: ^Vec_Add_CS_App) {
	...
	compute_pso_desc := d3d12.COMPUTE_PIPELINE_STATE_DESC {
		...
		CS = common.byte_code_from_blob(app.shaders["vecAddCS"]),
	}
	vec_add: ^d3d12.IPipelineState
	common.hr_panic(
		app.device->CreateComputePipelineState(
			&compute_pso_desc, d3d12.IPipelineState_UUID, common.ptr(&vec_add),
		),
		"CreateComputePipelineState(vecAdd)",
	)
	app.psos["vecAdd"] = vec_add
}

// C++: CrateApp::BuildFrameResources.
build_frame_resources :: proc(app: ^Vec_Add_CS_App) {
	pass_count :: 1
...
build_materials :: proc(app: ^Vec_Add_CS_App) {
...
build_render_items :: proc(app: ^Vec_Add_CS_App) {
	geo := app.geometries["shapeGeo"]

	world := d3d_math.scaling(2.0, 2.0, 2.0) * d3d_math.translation(0.0, 0.0, 0.0)
	tex_transform := d3d_math.scaling(1.0, 1.0, 1.0)
	add_render_item(
		app,
		.Opaque,
		world,
		tex_transform,
		common.material_lib_get(&app.mat_lib, "crate"),
		geo,
		geo.draw_args["box"],
	)
}
...
// C++: VecAddCS::BuildComputeBuffers.
build_compute_buffers :: proc(
	app: ^Vec_Add_CS_App,
	upload_batch: ^common.Resource_Upload_Batch,
) {
	data_a: [NUM_DATA_ELEMENTS]Data
	data_b: [NUM_DATA_ELEMENTS]Data
	...
	byte_size := u64(NUM_DATA_ELEMENTS * size_of(Data))
	output_desc := common.buffer_desc(byte_size)
	output_desc.Flags = {.ALLOW_UNORDERED_ACCESS}
	...
	common.hr_panic(
		app.device->CreateCommittedResource(
			...
			common.ptr(&app.output_buffer),
		),
		"CreateCommittedResource(VecAdd output)",
	)

	read_back_desc := common.buffer_desc(byte_size)
	read_back_heap := d3d12.HEAP_PROPERTIES{Type = .READBACK}
	common.hr_panic(
		app.device->CreateCommittedResource(
			&read_back_heap,
			{},
			...
			nil,
			d3d12.IResource_UUID,
			common.ptr(&app.read_back_buffer),
		),
		"CreateCommittedResource(VecAdd readback)",
	)
}

// C++: VecAddCS::BuildComputeDescriptors.
build_compute_descriptors :: proc(app: ^Vec_Add_CS_App) {
	app.buffer_index_a = common.next_free_index(&app.cbv_srv_uav_heap)
	app.buffer_index_b = common.next_free_index(&app.cbv_srv_uav_heap)
	app.buffer_output_index = common.next_free_index(&app.cbv_srv_uav_heap)

	common.create_buffer_srv(
		app.device,
		0,
		NUM_DATA_ELEMENTS,
		size_of(Data),
		app.input_buffer_a,
		common.cpu_handle(&app.cbv_srv_uav_heap, app.buffer_index_a),
	)
	common.create_buffer_srv(
		app.device,
		0,
		...
		size_of(Data),
		app.input_buffer_b,
		common.cpu_handle(&app.cbv_srv_uav_heap, app.buffer_index_b),
	)
	common.create_buffer_uav(
		app.device,
		0,
		NUM_DATA_ELEMENTS,
		size_of(Data),
		...
		app.output_buffer,
		nil,
		common.cpu_handle(&app.cbv_srv_uav_heap, app.buffer_output_index),
	)
}

// C++: VecAddCS::DoComputeWork. Dispatch once, copy the results to a readback
// buffer, verify them, and retain the book sample's results.txt output.
do_compute_work :: proc(app: ^Vec_Add_CS_App) {
	common.hr_panic(app.direct_cmd_list_alloc->Reset(), "CommandAllocator Reset(VecAdd)")
	common.hr_panic(
		app.command_list->Reset(app.direct_cmd_list_alloc, app.psos["vecAdd"]),
		"CommandList Reset(VecAdd)",
	)

	descriptor_heaps := [?]^d3d12.IDescriptorHeap{
		app.cbv_srv_uav_heap.heap,
		app.sampler_heap.heap,
	}
	app.command_list->SetDescriptorHeaps(len(descriptor_heaps), &descriptor_heaps[0])
	app.command_list->SetComputeRootSignature(app.compute_root_signature)

	dispatch_cb := Dispatch_CB {
		buffer_index_a = app.buffer_index_a,
		buffer_index_b = app.buffer_index_b,
		buffer_output_index = app.buffer_output_index,
	}
	mem_handle := common.allocate_constant(&app.linear_allocator, dispatch_cb)
	app.command_list->SetComputeRootConstantBufferView(
		u32(Compute_Root_Arg.DISPATCH_CBV),
		mem_handle.gpu_address,
	)
	app.command_list->Dispatch(1, 1, 1)

	to_copy_source := common.transition_barrier(
		app.output_buffer,
		{.UNORDERED_ACCESS},
		{.COPY_SOURCE},
	)
	app.command_list->ResourceBarrier(1, &to_copy_source)
	app.command_list->CopyResource(app.read_back_buffer, app.output_buffer)
	to_unordered_access := common.transition_barrier(
		app.output_buffer,
		{.COPY_SOURCE},
		{.UNORDERED_ACCESS},
	)
	app.command_list->ResourceBarrier(1, &to_unordered_access)

	common.hr_panic(app.command_list->Close(), "CommandList Close(VecAdd)")
	command_lists := [?]^d3d12.ICommandList{(^d3d12.ICommandList)(app.command_list)}
	app.command_queue->ExecuteCommandLists(len(command_lists), &command_lists[0])
	common.commit(&app.linear_allocator, app.command_queue)
	common.flush_command_queue(&app.base)

	mapped: rawptr
	common.hr_panic(app.read_back_buffer->Map(0, nil, &mapped), "Map(VecAdd readback)")
	results := ([^]Data)(mapped)[:NUM_DATA_ELEMENTS]

	builder := strings.builder_make(context.temp_allocator)
	defer strings.builder_destroy(&builder)
	for result, i in results {
		x := f32(i)
		assert(result.v1 == [3]f32{0, 2 * x, x})
		assert(result.v2 == [2]f32{x, -x})
		fmt.sbprintf(
			&builder,
			"(%g, %g, %g, %g, %g)\n",
			f64(result.v1[0]),
	...
	if err := os.write_entire_file("results.txt", strings.to_string(builder)); err != nil {
		common.report_error(fmt.tprintf("writing results.txt failed: %v", err))
		os.exit(1)
	}
	app.read_back_buffer->Unmap(0, nil)
}
