// Port of `Demos/C9_TexturedShapes` (TexturedShapesApp.cpp/.h) — the ch 8 LitShapes
// scene, textured: bricks on the box and cylinders, tiles on the floor, rock on the
// spheres. New over C9_Crate: per-item **texture transforms** (the floor tiles its UVs
// 8x8, the cylinders stretch bricks 1.5x2) flowing through gTexTransform, multiplied
// with the material's own MatTransform in the vertex shader. See C9_Crate for the
...
//   odin run odin_port/C9_TexturedShapes -debug
package c9_texturedshapes
...
main :: proc() {
	...
	app: Textured_Shapes_App

	// C++ member initializers (TexturedShapesApp.h).
	...
	// C++: TexturedShapesApp::Initialize().
...
// C++: TexturedShapesApp::OnResize.
on_resize :: proc(base: ^common.D3D_App) {
	app := (^Textured_Shapes_App)(base)
...
// C++: TexturedShapesApp::OnKeyboardInput — empty in this demo.
on_keyboard_input :: proc(app: ^Textured_Shapes_App) {
}

// C++: TexturedShapesApp::AnimateMaterials — empty in this demo.
animate_materials :: proc(app: ^Textured_Shapes_App) {
}

// C++: TexturedShapesApp::UpdateCamera.
update_camera :: proc(app: ^Textured_Shapes_App) {
...
// C++: TexturedShapesApp::UpdatePerObjectCB.
update_per_object_cb :: proc(app: ^Textured_Shapes_App) {
...
// C++: TexturedShapesApp::UpdateMaterialBuffer — the upload now carries the texture transform and
// the bindless texture indices alongside the shading constants.
update_material_buffer :: proc(app: ^Textured_Shapes_App) {
...
// C++: TexturedShapesApp::UpdateMainPassCB — identical to chapter 8's.
update_main_pass_cb :: proc(app: ^Textured_Shapes_App) {
...
// C++: TexturedShapesApp::UpdateImgui — the "Options" panel.
update_imgui :: proc(app: ^Textured_Shapes_App) {
...
// C++: TexturedShapesApp::Draw — the sampler heap is bound alongside the CbvSrvUav heap now.
...
// C++: TexturedShapesApp::DrawRenderItems.
draw_render_items :: proc(app: ^Textured_Shapes_App, ritems: []^Render_Item) {
...
// C++: TexturedShapesApp::OnMouseDown — ImGui gets first claim on the mouse.
...
// C++: TexturedShapesApp::OnMouseMove.
...
// C++: TexturedShapesApp::BuildCbvSrvUavDescriptorHeap — after ImGui claims its slot, every
...
build_cbv_srv_uav_descriptor_heap :: proc(app: ^Textured_Shapes_App) {
...
// C++: TexturedShapesApp::BuildRootSignature — same three parameters as chapter 8, plus the two
// SM 6.6 "directly indexed" flags that let shaders use ResourceDescriptorHeap[] and
// SamplerDescriptorHeap[].
build_root_signature :: proc(app: ^Textured_Shapes_App) {
...
// C++: TexturedShapesApp::BuildShadersAndInputLayout — BasicTex.hlsl.
build_shaders_and_input_layout :: proc(app: ^Textured_Shapes_App) {
...
// C++: TexturedShapesApp::BuildPSOs.
build_psos :: proc(app: ^Textured_Shapes_App) {
...
// C++: TexturedShapesApp::BuildFrameResources.
build_frame_resources :: proc(app: ^Textured_Shapes_App) {
	pass_count :: 1
...
// C++: TexturedShapesApp::BuildMaterials — MaterialLib::GetLib().Init(...).
build_materials :: proc(app: ^Textured_Shapes_App) {
	common.material_lib_init(&app.mat_lib, &app.tex_lib)
}

// C++: TexturedShapesApp::AddRenderItem — gains the texTransform parameter this chapter.
...
// C++: TexturedShapesApp::BuildRenderItems — the ch 8 scene with per-item texture
// transforms: bricks stretched on the box and cylinders, the floor tiled 8x8.
build_render_items :: proc(app: ^Textured_Shapes_App) {
	shape_geo := app.geometries["shapeGeo"]
	skull_geo := app.geometries["skullGeo"]

	bricks0 := common.material_lib_get(&app.mat_lib, "bricks0")
	tile0 := common.material_lib_get(&app.mat_lib, "tile0")
	rock0 := common.material_lib_get(&app.mat_lib, "rock0")
	skull_mat := common.material_lib_get(&app.mat_lib, "skullMat")

	world := d3d_math.scaling(2.0, 1.0, 2.0) * d3d_math.translation(0.0, 0.5, 0.0)
	add_render_item(app, .Opaque, world, d3d_math.scaling(1.0, 0.5, 1.0), bricks0, shape_geo, shape_geo.draw_args["box"])

	world = d3d_math.scaling(0.4, 0.4, 0.4) * d3d_math.translation(0.0, 1.0, 0.0)
	add_render_item(app, .Opaque, world, d3d_math.MAT4_IDENTITY, skull_mat, skull_geo, skull_geo.draw_args["skull"])

	add_render_item(app, .Opaque, d3d_math.MAT4_IDENTITY, d3d_math.scaling(8.0, 8.0, 1.0), tile0, shape_geo, shape_geo.draw_args["grid"])

	for i in 0 ..< 5 {
		z := -10.0 + f32(i) * 5.0
		brick_tex := d3d_math.scaling(1.5, 2.0, 1.0)
		add_render_item(app, .Opaque, d3d_math.translation(-5.0, 1.5, z), brick_tex, bricks0, shape_geo, shape_geo.draw_args["cylinder"])
		add_render_item(app, .Opaque, d3d_math.translation(+5.0, 1.5, z), brick_tex, bricks0, shape_geo, shape_geo.draw_args["cylinder"])
		add_render_item(app, .Opaque, d3d_math.translation(-5.0, 3.5, z), d3d_math.MAT4_IDENTITY, rock0, shape_geo, shape_geo.draw_args["sphere"])
		add_render_item(app, .Opaque, d3d_math.translation(+5.0, 3.5, z), d3d_math.MAT4_IDENTITY, rock0, shape_geo, shape_geo.draw_args["sphere"])
	}
}
