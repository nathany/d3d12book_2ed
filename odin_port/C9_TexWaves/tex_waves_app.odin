// Port of `Demos/C9_TexWaves` (TexWavesApp.cpp/.h) — the ch 8 hills-and-waves scene,
// textured: grass on the land, a **scrolling water texture** (AnimateMaterials nudges the
// water material's MatTransform translation every frame and re-dirties it — the first
// material that changes at runtime), and a floating wooden crate. The waves' UVs are
// derived from position each frame, mapping the grid onto [0,1]².
//
...
on_keyboard_input :: proc(app: ^Tex_Waves_App) {
}
...
animate_materials :: proc(app: ^Tex_Waves_App) {
...
update_camera :: proc(app: ^Tex_Waves_App) {
...
update_per_object_cb :: proc(app: ^Tex_Waves_App) {
...
update_material_buffer :: proc(app: ^Tex_Waves_App) {
...
update_main_pass_cb :: proc(app: ^Tex_Waves_App) {
...
update_waves :: proc(app: ^Tex_Waves_App) {
...
update_imgui :: proc(app: ^Tex_Waves_App) {
...
draw_render_items :: proc(app: ^Tex_Waves_App, ritems: []^Render_Item) {
...
build_cbv_srv_uav_descriptor_heap :: proc(app: ^Tex_Waves_App) {
...
build_root_signature :: proc(app: ^Tex_Waves_App) {
...
// C++: TexWavesApp::BuildShadersAndInputLayout — BasicTex.hlsl.
build_shaders_and_input_layout :: proc(app: ^Tex_Waves_App) {
...
build_psos :: proc(app: ^Tex_Waves_App) {
...
build_frame_resources :: proc(app: ^Tex_Waves_App) {
	pass_count :: 1
...
build_materials :: proc(app: ^Tex_Waves_App) {
...
// C++: TexWavesApp::BuildRenderItems — tiled water (5x5) and grass (8x8), plus the
// floating crate.
build_render_items :: proc(app: ^Tex_Waves_App) {
...
// C++: TexWavesApp::BuildWaveGeometry — static 32-bit indices, matching the book.
...
// C++: TexWavesApp::BuildShapeGeometry — just the crate's box in this demo.
