// Port of `Demos/C6_Box` (BoxApp.cpp/.h) — chapter 6's first real draw: a colored box,
// vertex/index buffers uploaded to the default heap, two constant buffers (per-object
// world, per-pass viewProj) bound through CBV descriptor tables, BasicColor.hlsl compiled
// at startup with DXC (SM 6.6), solid + wireframe PSOs, and an orbit camera on the mouse
// (left drag = orbit, right drag = zoom).
//
// Build & run FROM THE REPO ROOT (shader path + DXC DLLs are resolved relative to it):
//   odin run odin_port/C6_Box -debug
// First time: copy dxcompiler.dll + dxil.dll next to the exe — see odin_port/README.md.
package c6_box
...
// C++: struct ObjectConstants { XMFLOAT4X4 World; };  (identity-initialized in main)
...
Box_App :: struct {
	...
	// Spherical coordinates of the orbiting camera.
	theta:               f32,
	phi:                 f32,
	radius:              f32,
...
main :: proc() {
	...
	app.world = d3d_math.MAT4_IDENTITY
	...
	// C++ kicks the upload off asynchronously here (std::future) and blocks on it at the
	// end of Initialize; the port executes and waits in one step — same observable
	// behavior, no threads (see upload_batch.odin's provenance header).
...
update :: proc(base: ^common.D3D_App) {
	...
	// (transpose before upload: HLSL cbuffers default to column-major storage.)
	obj_constants := Object_Constants {
		world = linalg.transpose(app.world),
	}
	common.copy_data(&app.object_cb, 0, obj_constants)
...
// C++: BoxApp::UpdateImgui — the "Options" panel.
update_imgui :: proc(app: ^Box_App) {
	...
	// (C++ has a GraphicsMemoryStatistics section here — DirectXTK12's GraphicsMemory,
	// which this port replaces with its own upload arena in ch 7; its stats join then.)
...
draw :: proc(base: ^common.D3D_App) {
	...
	base.command_list->SetGraphicsRootDescriptorTable(
		...
		common.gpu_handle(&app.cbv_srv_uav_heap, app.box_cb_heap_index),
	)
	base.command_list->SetGraphicsRootDescriptorTable(
...
// C++: BoxApp::OnMouseDown — ImGui gets first claim on the mouse.
...
// C++: BoxApp::BuildCbvSrvUavDescriptorHeap.
build_cbv_srv_uav_descriptor_heap :: proc(app: ^Box_App) {
...
// C++: BoxApp::BuildConstantBuffers.
build_constant_buffers :: proc(app: ^Box_App) {
	app.box_cb_heap_index = common.next_free_index(&app.cbv_srv_uav_heap)
	app.pass_cb_heap_index = common.next_free_index(&app.cbv_srv_uav_heap)

	element_count :: 1
	is_constant_buffer :: true
	common.upload_buffer_init(&app.object_cb, app.device, element_count, is_constant_buffer)
	common.upload_buffer_init(&app.pass_cb, app.device, element_count, is_constant_buffer)
	...
	cb_obj_element_offset := 0
	obj_cb_address :=
		app.object_cb.upload_buffer->GetGPUVirtualAddress() +
		u64(cb_obj_element_offset) * u64(obj_cb_byte_size)
	...
	app.device->CreateConstantBufferView(
		&cbv_obj,
		common.cpu_handle(&app.cbv_srv_uav_heap, app.box_cb_heap_index),
	)
...
// C++: BoxApp::BuildRootSignature.
build_root_signature :: proc(app: ^Box_App) {
...
// C++: BoxApp::BuildShadersAndInputLayout.
build_shaders_and_input_layout :: proc(app: ^Box_App) {
...
// C++: BoxApp::BuildBoxGeometry(device, uploadBatch).
build_box_geometry :: proc(app: ^Box_App, upload_batch: ^common.Resource_Upload_Batch) {
	...
	// System-memory copies (kept for later chapters: picking reads them back).
