// Port of `Demos/C14_BezierPatch` (BezierPatchApp.cpp/.h): a 16-control-point bicubic
// Bézier patch tessellated by hull/domain shaders and rasterized as a wireframe.
//
// Build & run FROM THE REPO ROOT (shader path + DXC DLLs are resolved relative to it):
//   odin run odin_port/C14_BezierPatch -debug
package c14_bezier_patch
...
main :: proc() {
	...
	app: Bezier_Patch_App

	// C++ member initializers (BezierPatchApp.h) — the initial view of the patch.
...
// C++: BezierPatchApp::OnResize.
on_resize :: proc(base: ^common.D3D_App) {
	app := (^Bezier_Patch_App)(base)
...
// C++: BezierPatchApp::OnKeyboardInput — empty in this demo.
on_keyboard_input :: proc(app: ^Bezier_Patch_App) {
}

// C++: BezierPatchApp::AnimateMaterials — empty in this demo.
animate_materials :: proc(app: ^Bezier_Patch_App) {
}

// C++: BezierPatchApp::UpdateCamera.
update_camera :: proc(app: ^Bezier_Patch_App) {
...
// C++: BezierPatchApp::UpdatePerObjectCB.
update_per_object_cb :: proc(app: ^Bezier_Patch_App) {
	...
	for ri in app.all_ritems {
		...
		ri.object_cb.misc_float4[0] = app.tess_factor
...
// C++: BezierPatchApp::UpdateMaterialBuffer — the upload carries the texture transform and
// the bindless texture indices alongside the shading constants.
update_material_buffer :: proc(app: ^Bezier_Patch_App) {
...
// C++: BezierPatchApp::UpdateMainPassCB — frame constants for the tessellation shaders.
update_main_pass_cb :: proc(app: ^Bezier_Patch_App) {
...
// C++: BezierPatchApp::UpdateImgui — the "Options" panel.
update_imgui :: proc(app: ^Bezier_Patch_App) {
	...
	im.SliderFloat("Tess Factor", &app.tess_factor, 1.0, 64.0)
...
// C++: BezierPatchApp::Draw — bind the sampler and CbvSrvUav heaps.
...
// C++: BezierPatchApp::DrawRenderItems.
draw_render_items :: proc(app: ^Bezier_Patch_App, ritems: []^Render_Item) {
...
// C++: BezierPatchApp::OnMouseDown — ImGui gets first claim on the mouse.
...
// C++: BezierPatchApp::OnMouseMove — orbit and zoom around the patch.
...
// C++: BezierPatchApp::BuildCbvSrvUavDescriptorHeap — after ImGui claims its slot, every
...
build_cbv_srv_uav_descriptor_heap :: proc(app: ^Bezier_Patch_App) {
...
// C++: BezierPatchApp::BuildRootSignature — object/pass CBVs, a material SRV, and the two
// SM 6.6 "directly indexed" flags that let shaders use ResourceDescriptorHeap[] and
// SamplerDescriptorHeap[].
build_root_signature :: proc(app: ^Bezier_Patch_App) {
...
// C++: BezierPatchApp::BuildShadersAndInputLayout — BezierTessellation.hlsl.
build_shaders_and_input_layout :: proc(app: ^Bezier_Patch_App) {
	...
	app.shaders["tessVS"] = common.compile_shader("Shaders/BezierTessellation.hlsl", vs_args[:])
	app.shaders["tessHS"] = common.compile_shader("Shaders/BezierTessellation.hlsl", hs_args[:])
	app.shaders["tessDS"] = common.compile_shader("Shaders/BezierTessellation.hlsl", ds_args[:])
	app.shaders["tessPS"] = common.compile_shader("Shaders/BezierTessellation.hlsl", ps_args[:])
...
// C++: BezierPatchApp::BuildPSOs.
build_psos :: proc(app: ^Bezier_Patch_App) {
...
// C++: BezierPatchApp::BuildFrameResources.
build_frame_resources :: proc(app: ^Bezier_Patch_App) {
	pass_count :: 1
...
// C++: BezierPatchApp::BuildMaterials — MaterialLib::GetLib().Init(...).
build_materials :: proc(app: ^Bezier_Patch_App) {
	common.material_lib_init(&app.mat_lib, &app.tex_lib)
}

// C++: BezierPatchApp::AddRenderItem — record the patch transform and material.
...
// C++: BezierPatchApp::BuildRenderItems — one sixteen-control-point Bezier patch.
build_render_items :: proc(app: ^Bezier_Patch_App) {
	...
	app.all_ritems[len(app.all_ritems) - 1].primitive_type = ._16_CONTROL_POINT_PATCHLIST
}

// C++: BezierPatchApp::BuildQuadPatchGeometry — sixteen-control-point Bezier patch.
