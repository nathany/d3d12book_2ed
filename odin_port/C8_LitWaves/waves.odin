// Port of `Demos/C8_LitWaves/Waves.h/.cpp` (Frank Luna) — identical to C7_Waves' copy
// (the C++ duplicates the file per demo too; this chapter starts *using* the normals the
// simulation was already computing). Performs the calculations for the wave simulation;
// after the update, the client must copy the current solution into vertex buffers for
// rendering. This file only does the calculations, it does not do any drawing.
//
